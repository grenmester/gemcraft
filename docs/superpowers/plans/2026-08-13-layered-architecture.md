...
- **No test assertion body may be modified.** Import paths change; nothing else. If a test needs a real edit to pass, the move broke behaviour — stop, revert, diagnose. Never edit a test to match new behaviour.
...
- **The test count starts at 622 and ends at 623.** The single layer-rule test in Task 11 is the only test added. Any other change in count means a test was lost or duplicated in a move and must be explained before merge.
