...
- **Across Tasks 1–12 the test count starts at 622 and ends at 623.** The single layer-rule test in Task 11 is the only test added. Any other change in count means a test was lost or duplicated in a move and must be explained before merge. Task 13 is the sole exception: it removes tests deliberately, under the evidence criteria stated in that task.
